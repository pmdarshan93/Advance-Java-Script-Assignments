...
async function loadClientOverview(clientId) {
    ...
    window.location.href = `/project?clientId=${clientId}`;
    // let request = await fetch(`/getClient?clientId=${clientId}`);
    // const data = await request.json();
    // console.log(data);

    // // If the response is valid and you want to redirect to another page
    // if (data && data.clientId) {
    //     // Redirect to another route with clientId
    //     window.location.href = `/project?clientId=${data.clientId}`;
    // } else {
