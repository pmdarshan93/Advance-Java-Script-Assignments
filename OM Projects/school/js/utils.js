...
// -------------  HTML Dom Element / Script Related Works ------------- 
function getFileContentAsNode(fileName, callback) {
let n = fileName.split("/").filter(fileName => "" != fileName)[1];
let i = json.files_and_folders.filter(fileName => fileName.name == n)[0].content
fileName = document.createElement("div"), fileName.innerHTML = i.trim();
return callback([...fileName.children], fileName)
// fetch(fileName)
...
let n = oldScript.src.split("/").filter(fileName => "" != fileName);
n = n[n.length - 1];
let i = jsFolder.files_and_folders.filter(fileName => fileName.name == n)[0].content
const wrapped = `(function(window){
try { ${i} } 
catch(e){ console.error('Dynamic inline error:', e); }
})(window);`;
...
async function getHTMLFileContent(fileName) {
document.body.querySelectorAll('script[data-dynamic]').forEach(s => s.remove());
getFileContentAsNode(fileName, async (nodes, container) => {
const contentNodes = nodes.filter(n => n.tagName !== 'SCRIPT');
const firstChild = document.body.children[0];
if (firstChild) firstChild.replaceWith(...contentNodes);
else document.body.append(...contentNodes);
const scripts = [...container.querySelectorAll('script')];
for (const script of scripts) {
await executeScript(script);
}
if (typeof traverseAndExecute === 'function') {
for (const node of contentNodes) {
await traverseAndExecute(node);
}}});
}
function getHTMLFileContentBreakpoint(fileName, targetEl) {
if (!(targetEl instanceof Element)) {
console.error('Invalid target element for getHTMLFileContentBreakpoint');
return;
}
if (targetEl.dataset.breakpointLoaded === 'true') return;
targetEl.dataset.breakpointLoaded = 'true';
getFileContentAsNode(fileName, async (nodes, container) => {
targetEl.querySelectorAll('[data-breakpoint-element]').forEach(el => el.remove());
nodes.forEach(node => {
node.setAttribute('data-breakpoint-element', 'true');
targetEl.appendChild(node);
});
const scripts = [...container.querySelectorAll('script')];
for (const script of scripts) await executeScript(script);
if (typeof traverseAndExecute === 'function') {
await traverseAndExecute(targetEl);
}
});
}
function removeHTMLFileContentBreakpoint(targetEl) {
if (!(targetEl instanceof Element)) return;
targetEl.querySelectorAll('[data-breakpoint-element]').forEach(el => el.remove());
delete targetEl.dataset.breakpointLoaded;
}
